...
const UNAVAILABLE_USERNAME_MESSAGE =
  "The chosen username is unavailable, please choose another one";
const UNAUTHORIZED_MESSAGE = "Incorrect username or password";
...
class UserService {
  ...
  async getBalance(authorization: string | undefined): Promise<number> {
    ...
    if (!user || !account)
      throw new NotFoundError("User or account not found.");
